...
import stirling.software.SPDF.config.security.saml.Saml2LogoutSuccessHandler;
...
public class SecurityConfiguration {
            ...
            if (applicationProperties.getSecurity().getSAML() != null
                ...
                http.saml2Login(
                                saml2 -> {
                                    ...
                                    saml2.successHandler(
                                            new CustomSAMLAuthenticationSuccessHandler(
                                                    loginAttemptService,
                                                    userService,
                                                    applicationProperties));
                                    saml2.failureHandler(
                                            new CustomSAMLAuthenticationFailureHandler());
                                })
                        .saml2Logout(
                                saml2LogoutConfigurer ->
                                        saml2LogoutConfigurer.logoutRequest(
                                                request -> request.logoutUrl("/logout/saml2/slo")))
                        .saml2Metadata((saml2) -> saml2.metadataUrl("/saml/metadata"))
                        .logout(
                                logout ->
                                        logout.logoutSuccessHandler(
                                                new Saml2LogoutSuccessHandler(
                                                        applicationProperties,
                                                        relyingPartyRegistrationRepository)))
                        .addFilterBefore(
    ...
    public AuthenticationProvider daoAuthenticationProvider() {
        ...
        provider.setUserDetailsService(userDetailsService); // Ihr UserDetailsService
        provider.setPasswordEncoder(passwordEncoder()); // Ihr PasswordEncoder
        return provider;
    }
